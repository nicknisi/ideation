...
**pi front door:** in the pi port, a fresh intake mines the model first (scout → candidates + grail → advisor → `ask()` gate) and interviews only the declared-ignorance list, per the engine's *Mine first (pi front door)* section; a picked option seeds gate evidence, reject-all/dismiss falls back to the classic interview, and the intake counts every user-facing question for Phase 3 to persist. Claude Code runs the same mining flow conversationally — the Agent tool fans out the candidates and holy grail, an advisor ranks them (its JSON validated before the gate), and an `AskUserQuestion` gates the pick, per the engine's *Claude Code branch* — then routes into the identical ignorance interview. Mined items are tagged `source: 'mined'` so the contract renders a model-assumed badge (see `${CLAUDE_PLUGIN_ROOT}/references/harness-compat.md` § 4).
...
7. **Present the Critic digest and a contract summary, then ask for approval and routing in one call.** Only ask once `contract.html` was actually generated and opened — never ask the user to approve a contract they haven't seen. Present in the terminal, in order:
   ...
   **pi review surface (full-review path):** in the pi port, when the routing answer is **full review**, route the contract approval through the `ideation_review` tool rather than the terminal button — call it with the project's `docs/ideation` base directory and `{slug}`. It opens the annotatable `contract.html` in a browser (block-pinned comments, an approve/deny bar) and blocks until the reviewer decides, returning `{ decision, reasons?, comments }`: **approved** flips `contract-data.json` to Approved on the surface — continue to Phase 4; **denied** writes reasons to `feedback-{date}.json` — treat as "Needs changes" and fold the feedback into the revision; **dismissed** (closed tab or idle timeout) → fall back to the terminal `AskUserQuestion` above, unchanged. Claude Code, and any pi where the review extension is not loaded, use the terminal question directly (see `${CLAUDE_PLUGIN_ROOT}/references/harness-compat.md` § 4). The express finish bypasses the surface entirely.
