...
- `--strict` — modifies `--headless` to fail **closed** at the two decision points that otherwise fail open: scout HOLD → report the gap analysis and stop (do not build); reviewer failure/empty/no verdict → do not fall back to validation-only — report FAIL and stop without committing. Set by autopilot for express-approved contracts (`approvalMode: "express"` in `contract-data.json`), whose artifacts no human reviewed — the auto-proceed defaults are calibrated for reviewed artifacts and don't apply.
...
- **HOLD** (Scope clarity not-ready, or < 4/5 gates ready): present the gap analysis via `AskUserQuestion` (headless: auto-select "Proceed anyway"; headless + `--strict`: report the gap analysis and stop — an under-specified spec that no human reviewed is a stop condition, not a proceed condition):
...
**If the reviewer fails, returns empty, or has no verdict line:** fall back to validation-only mode (validation results are sufficient), warn that review was skipped, and continue to commit. **With `--strict`:** do not fall back — report FAIL (reviewer unavailable) with the validation results and stop without committing; unreviewed code from an unreviewed spec never lands silently.
