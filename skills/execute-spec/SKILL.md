...
**Headless/strict resolution matrix** — the prose owner of these semantics; autopilot and ideation's express finish point here rather than restating them. The **executable** owner is the workflow engine (`workflows/execute-contract.mjs`), which implements the same matrix for engine-dispatched phases and is covered row-by-row by `workflows/execute-contract.smoke.test.mjs`. Two owners, deliberately: they cross a context boundary. When they disagree, the tests are right and this table is stale — the original defect (strict specified to fail closed, actually improvising) shipped exactly because prose was the only owner.

| Decision point                                          | `--headless`                                                       | `--headless --strict`                            |
| ------------------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------ |
| Spec selection                                          | first unblocked                                                    | same                                             |
| Scout HOLD                                              | proceed anyway; a `SCOUT HOLD` warning leads the phase result      | report the gap analysis, stop — nothing built    |
| Reviewer fails / empty / no verdict                     | commit validation-only, degradation surfaced structurally (see Review) | report FAIL, stop — nothing committed            |
| Review FAIL at cycle 3 (incl. refuted-then-maintained)  | report FAIL, stop — no commit                                      | same                                             |
...
3. **No tasks** — glob `./docs/ideation/*/spec-phase-*.md` **and** `./docs/ideation/*/spec.md` (single-spec projects emit a bare `spec.md`). Multiple matches → `AskUserQuestion` to pick (headless: first unblocked).
...
- **HOLD** (Scope clarity not-ready, or < 4/5 gates ready): present the gap analysis via `AskUserQuestion` (headless/strict: per the resolution matrix above):
