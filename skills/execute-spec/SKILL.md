...
- **HOLD** (Scope clarity not-ready, or < 4/5 gates ready): present the gap analysis via `AskUserQuestion` (headless/strict: per the gate-behavior table in `workflows/README.md`):
...
On an interactive run, after the completion report, run the Learning Capture
step — bounded to the just-finished project's `implementation-notes-phase-*.html`
and this run's noted gap-decisions — per
