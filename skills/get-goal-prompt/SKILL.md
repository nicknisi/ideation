...
confirm the Workflow feature is available. One **caveat, not a gate** — and honestly
labeled: this detail was derived from Claude Code binary strings and could not be
verified from this repo — `/goal` appears to require a trusted workspace and may refuse
to run when hooks are restricted. If the paste is rejected, that is the likely reason;
it is a property of the CLI session, not of this project.
